...
func TestServeReturnsWhenContextCancelled(t *testing.T) {
	...
	if _, err := os.Stat(s.SocketPath()); !os.IsNotExist(err) {
		t.Fatal("socket file should be removed once Serve returns")
	}
}

func TestCloseReleasesHandlerBlockedOnContext(t *testing.T) {
	dir := t.TempDir()
	s := socket.New(dir, "test")
	...
	go func() { callDone <- c.Call(context.Background(), "wait", nil, nil) }()
	time.Sleep(100 * time.Millisecond) // let the handler start waiting

	s.Close()
	select {
	case <-released:
	case <-time.After(3 * time.Second):
		t.Fatal("handler context was not cancelled by Close")
	}
	select {
	case <-served:
	case <-time.After(3 * time.Second):
...
func TestOpenRefusedWhileServing(t *testing.T) {
	dir := t.TempDir()
	s := socket.New(dir, "test")
	if err := s.Open(); err != nil {
		t.Fatal(err)
	...
	if err := s.Open(); err == nil {
		t.Fatal("Open while serving must fail")
	}
	s.Close()
	<-served
	// After Serve returned, reopening is allowed.
	if err := s.Open(); err != nil {
		t.Fatalf("reopen after Serve returned: %v", err)
	}
	s.Close()
}
...
func TestCloseFromHandlerDoesNotDeadlock(t *testing.T) {
	dir := t.TempDir()
	s := socket.New(dir, "test")
	s.Handle("shutdown", func(ctx context.Context, _ json.RawMessage) (any, error) {
		s.Close()
		return "bye", nil
	})
	if err := s.Open(); err != nil {
	...
	case <-time.After(3 * time.Second):
		t.Fatal("Serve did not return after Close from a handler")
	}
}
