...
// Colors is the palette used by a Theme.
type Colors struct {
...
// DefaultColors is the palette shared by the wizard and the app framework.
...
// Copy holds the user-facing strings a Theme renders.
type Copy struct {
	Title     string
	Move      string
	Toggle    string
...
// DefaultCopy is the English copy.
...
// Theme bundles colors, copy and layout settings.
type Theme struct {
	Colors    Colors
	Copy      Copy
	Indent    string
...
// DefaultTheme is the theme used by the built-in steps and components.
...
// Styles derives the lipgloss styles for the theme.
