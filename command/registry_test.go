...
func TestZeroValueRegistryIsUsable(t *testing.T) {
	...
	if r.IsOneShot("x") {
		t.Fatal("empty registry must not report commands")
	}
	if handled, _ := r.Dispatch(context.Background(), "x", nil); handled {
		t.Fatal("empty registry must not dispatch")
	}
	r.Register(command.Handler{Name: "x", Run: func(context.Context, []string) int { return 7 }})
	handled, code := r.Dispatch(context.Background(), "x", nil)
	if !handled || code != 7 {
		t.Fatalf("Dispatch after Register on zero value: handled=%v code=%d", handled, code)
	}
}
