...
func TestPrintChanges(t *testing.T) {
	...
	installer.PrintChanges(&b, nil, harness.Scope{})
	if !strings.Contains(b.String(), "Nothing to change") {
		t.Fatalf("empty plan: %q", b.String())
	}
	b.Reset()
	installer.PrintChanges(&b, []harness.Change{
		{HarnessID: "a", Name: "Alpha", Action: "create", Path: "/p/a.json"},
		{HarnessID: "b", State: harness.ApplyState("unavailable"), Reason: "unknown harness"},
		{HarnessID: "c", Name: "Gamma", State: harness.ApplyNoop},
	}, harness.ProjectScopeDir("/proj"))
	out := b.String()
	for _, want := range []string{"Project scope: /proj", "Alpha", "would create", "/p/a.json", "b ", "unavailable: unknown harness", "Gamma", "already registered"} {
		if !strings.Contains(out, want) {
