...
func TestNextPageHintAtFirstPage(t *testing.T) {
	meta := render.PageMeta{Page: 1, Total: 100, TotalPages: 3}
	hint := render.NextPageHint(meta)
	if hint != " Next: page=2." {
		t.Fatalf("expected page=2 hint after page 1, got %q", hint)
	}
}

func TestNextPageHintMatchesBudgetPaginateIndexing(t *testing.T) {
	// budget.Paginate accepts requestedPage in 1..totalPages; the hint must
	// name a page that Paginate would serve.
	meta := render.PageMeta{Page: 2, Total: 100, TotalPages: 3}
	if got := render.NextPageHint(meta); got != " Next: page=3." {
		t.Fatalf("got %q", got)
	}
	if got := render.NextPageHint(render.PageMeta{Page: 0, TotalPages: 3}); got != "" {
		t.Fatalf("page 0 is not a valid 1-indexed page, got %q", got)
	}
}

func TestNextPageHintAtLastPage(t *testing.T) {
	meta := render.PageMeta{Page: 3, Total: 100, TotalPages: 3}
	hint := render.NextPageHint(meta)
	if hint != "" {
...
func TestNextPageHintSinglePage(t *testing.T) {
	meta := render.PageMeta{Page: 1, Total: 5, TotalPages: 1}
	hint := render.NextPageHint(meta)
	if hint != "" {
...
type valueErr struct{ id string }

func (e valueErr) Error() string { return "value error " + e.id }

func TestClassifyMatchesValueReceiverError(t *testing.T) {
	table := []render.ErrorMapping{
		{Target: valueErr{}, Code: render.CodeConflict, Hint: "value hint"},
	}
	e := render.Classify(fmt.Errorf("wrapped: %w", valueErr{id: "x"}), table)
	if e.Code != render.CodeConflict {
		t.Fatalf("expected value-type target to match, got code %q", e.Code)
	}
	if e.Hint != "value hint" {
		t.Fatalf("expected hint, got %q", e.Hint)
	}
}

func TestClassifySkipsTargetsThatAreNotErrors(t *testing.T) {
	table := []render.ErrorMapping{
		{Target: "not an error", Code: render.CodeConflict},
		{Target: 42, Code: render.CodeConflict},
		{Target: nil, Code: render.CodeConflict},
		{Target: &notFoundErr{}, Code: render.CodeNotFound},
	}
	e := render.Classify(&notFoundErr{name: "n"}, table)
	if e.Code != render.CodeNotFound {
