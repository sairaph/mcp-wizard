...
func TestSearchCursorFollowsIntoViewport(t *testing.T) {
	m := search.New("")
	m.Update(tea.WindowSizeMsg{Width: 80, Height: 16}) // viewport height 10
	var results []search.Result
	for i := 0; i < 40; i++ {
		results = append(results, search.Result{Label: fmt.Sprintf("res%02d", i)})
	}
	m.SetResults(results)
	for i := 0; i < 25; i++ {
		m.Update(tea.KeyMsg{Type: tea.KeyDown})
	}
	if !strings.Contains(m.View(), "res25") {
		t.Fatalf("cursor result must be visible, view:\n%s", m.View())
	}
	for i := 0; i < 25; i++ {
		m.Update(tea.KeyMsg{Type: tea.KeyUp})
	}
	if !strings.Contains(m.View(), "res00") {
		t.Fatalf("first result must be visible again, view:\n%s", m.View())
	}
}
