...
func TestListCursorFollowsIntoViewport(t *testing.T) {
	var items []list.Item
	for i := 0; i < 40; i++ {
		items = append(items, list.Item{Label: fmt.Sprintf("item%02d", i), Detail: "d"})
	}
	m := list.New("", items, 0, 0, 0)
	m.Update(tea.WindowSizeMsg{Width: 80, Height: 14}) // viewport height 10, 2 lines per item
	...
	if !strings.Contains(m.View(), "item30") {
		t.Fatalf("cursor item must be visible, view:\n%s", m.View())
	}
	m.Update(tea.KeyMsg{Type: tea.KeyPgUp})
	if m.Cursor != 20 {
		t.Fatalf("pgup: Cursor = %d, want 20", m.Cursor)
	}
	if !strings.Contains(m.View(), "item20") {
		t.Fatalf("cursor item must be visible after pgup, view:\n%s", m.View())
	}
	m.Update(tea.KeyMsg{Type: tea.KeyHome})
	if !strings.Contains(m.View(), "item00") {
		t.Fatalf("home must show the first item, view:\n%s", m.View())
	}
}
