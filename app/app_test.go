...
func TestHandleGlobalKeysOtherKeyNotHandled(t *testing.T) {
	m := app.AppModel{}
	handled, cmd := m.HandleGlobalKeys(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune{'q'}})
	if handled || cmd != nil {
		t.Fatal("expected plain keys to pass through")
	}
	if m.Quit {
		t.Fatal("plain key must not set Quit")
	}
}
...
func TestScrollIntoView(t *testing.T) {
	...
	app.ScrollIntoView(&vp, 3, 1)
	if vp.YOffset != 0 {
		t.Fatalf("line inside window must not scroll, YOffset=%d", vp.YOffset)
	}
	app.ScrollIntoView(&vp, 9, 1)
	if vp.YOffset != 5 {
		t.Fatalf("expected YOffset 5 so line 9 is the last visible line, got %d", vp.YOffset)
	}
	app.ScrollIntoView(&vp, 20, 2)
	if vp.YOffset != 17 {
		t.Fatalf("expected YOffset 17 so lines 20-21 are visible, got %d", vp.YOffset)
	}
	app.ScrollIntoView(&vp, 2, 1)
	if vp.YOffset != 2 {
		t.Fatalf("scrolling up must land the line at the top, got %d", vp.YOffset)
	}
	app.ScrollIntoView(nil, 0, 1)
}

func TestHandleGlobalKeysWindowSize(t *testing.T) {
	m := app.AppModel{}
	handled, _ := m.HandleGlobalKeys(tea.WindowSizeMsg{Width: 120, Height: 40})
	if handled {
